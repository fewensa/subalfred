...
pub use submetadatan::SubmetadatanError as MetadataError;
...
pub enum ArrayBytesError {
	...
	#[error("Parse int error `{}`", message)]
	ParseIntError { message: String },
}
...
impl From<RawArrayBytesError> for ArrayBytesError {
	fn from(e: RawArrayBytesError) -> Self {
		match e {
			...
			RawArrayBytesError::ParseIntError(error) =>
				Self::ParseIntError { message: format!("{}", error) },
		}
	}
}
